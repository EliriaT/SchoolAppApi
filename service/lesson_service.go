...
type LessonService interface {
	...
	// output schedule
	GetLessons(ctx context.Context, authToken *token.Payload) (response []dto.LessonResponse, err error)
...
// When a lesson is created, course's date are updated
func (c *lessonService) CreateLesson(ctx context.Context, authToken *token.Payload, req dto.CreateLessonRequest) (dto.LessonResponse, error) {
	...
	_, err = c.db.UpdateCourseDates(ctx, db.UpdateCourseDatesParams{Dates: courseDates})
	if err != nil {
		return dto.LessonResponse{}, err
	}
...
func (c *lessonService) GetLessons(ctx context.Context, authToken *token.Payload) (response []dto.LessonResponse, err error) {
	...
	if CheckRolePresence(authToken.Role, c.roles[Student].ID) {
		...
		for _, lesson := range schedule {
			respLesson := dto.NewLessonsResponse(db.Lesson{lesson.ID, lesson.Name_2, lesson.CourseID, lesson.TeacherID, lesson.StartHour, lesson.EndHour, lesson.WeekDay, lesson.Classroom, lesson.CreatedBy, lesson.UpdatedBy, lesson.CreatedAt, lesson.UpdatedAt})
			respLesson.TeacherID = lesson.TeacherID
			respLesson.ClassID = lesson.ClassID
			response = append(response, respLesson)
		}
		return response, err
	}
